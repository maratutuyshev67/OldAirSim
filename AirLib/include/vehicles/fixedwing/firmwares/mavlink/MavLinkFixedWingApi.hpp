...
#ifndef msr_airlib_MavLinkFixedWingController_hpp
#define msr_airlib_MavLinkFixedWingController_hpp
...
class MavLinkFixedWingApi : public FixedWingApiBase
{
public: //methods
    virtual ~MavLinkFixedWingApi()
    {
        closeAllConnection();
    ...
    void start_telemtry_thread()
    {
        ...
        this->telemetry_thread_ = std::thread(&MavLinkFixedWingApi::telemtry_thread, this);
    }
...
protected: //methods
    ...
    {
        //defaults should be added for PX4 generic fixedwing.
        static const FixedWingApiParams vehicle_params_;
...
protected: //methods
    virtual void connect()
    {
        if (!connecting_) {
            ...
            this->connect_thread_ = std::thread(&MavLinkFixedWingApi::connect_thread, this);
        }
    }
...
private: //methods
    ...
    {
        /* bug: I removed this releaseControl because it makes back-to-back move operations less smooth.
    ...
    virtual void normalizeControls()
    {
        //if controls are in not in 0-1 range then they are in -1 to 1 range in which case
        //we normalize them to 0 to 1 for PX4, not sure if this is needed
        //do we want the tla to be defined as a control surface as well?
        if (!is_controls_0_1_) {
            // change -1 to 1 to 0 to 1.
            for (size_t i = 0; i < Utils::length(control_surfaces_); ++i) {
                control_surfaces_[i] = (control_surfaces_[i] + 1.0f) / 2.0f;
            }
        }
        else {
            ...
            for (size_t i = 0; i < Utils::length(control_surfaces_); ++i) {
                control_surfaces_[i] = Utils::clip(0.8f * control_surfaces_[i] + 0.20f, 0.0f, 1.0f);
            }
        }
    }
    ...
    {
        if (connection_ == nullptr)
            throw std::domain_error("MavLinkFixedWing requires connection object to be set before createProxy call");
    ...
    void createMavConnection(const AirSimSettings::MavLinkConnectionInfo& connection_info)
    {
        ...
        //Uncomment below for sending images over MavLink --> but its not defined!
    ...
    void processMavMessages(const mavlinkcom::MavLinkMessage& msg)
    {
        if (msg.msgid == HeartbeatMessage.msgid) {
            ...
            if (!actuators_message_supported_) {
                ...
                control_surfaces_[0] = HilControlsMessage.roll_ailerons;
            ...
            for (auto i = 0; i < 8; ++i) {
                if (isarmed) {
                    control_surfaces_[i] = HilActuatorControlsMessage.controls[i];
                }
                else {
                    control_surfaces_[i] = 0;
                }
...
protected: //variables
    ...
    static const int ControlSurfacesCount = 4;

    const SensorCollection* sensors_;
    mutable std::mutex hil_controls_mutex_;
    AirSimSettings::MavLinkConnectionInfo connection_info_;
    float control_surfaces_[ControlSurfacesCount];
...
private: //variables
    ...
    std::shared_ptr<FixedWingApiBase> mav_vehicle_control_;
