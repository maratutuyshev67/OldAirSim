	...
	namespace airlib
	{
		...
		public: // methods
			...
			virtual void update(float dt=0) override
			{
				...
				msr::airlib::PhysicsBodyVertex::update(dt);
		...
		private: // methods
			...
			{
				aileron_deflection_ = (30 * (std::numbers::pi / 180)) * controls_.at(0).getOutput().control_deflection;
				elevator_deflection_ = (25 * (std::numbers::pi / 180)) * controls_.at(1).getOutput().control_deflection;
				rudder_deflection_ = (20 * (std::numbers::pi / 180)) * controls_.at(3).getOutput().control_deflection;
